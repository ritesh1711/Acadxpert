  ...
  return (
    <div className="fixed right-4 bottom-4 w-[380px] bg-white border border-gray-200 rounded-xl shadow-2xl flex flex-col overflow-hidden z-[999999]">
      ...
      <div className="flex-1 overflow-y-auto px-4 py-3 bg-gray-50" style={{ minHeight: 220, maxHeight: 300 }}>
        ...
        {messages.map((m, idx) => (
          <div key={idx} className={`flex items-end mb-2 ${m.role === 'user' ? 'justify-end' : 'justify-start'}`}>
            {m.role !== 'user' && getAvatar(m.role)}
            <div className={`px-4 py-2 rounded-2xl shadow-sm max-w-[75%] text-sm ${m.role === 'user' ? 'bg-blue-600 text-white' : m.role === 'ai' ? 'bg-purple-100 text-gray-900' : 'bg-gray-200 text-gray-900'}`}>
              {m.text}
              <div className="text-xs text-gray-400 mt-1 text-right">{m.time ? formatTime(m.time) : ''}</div>
            </div>
            {m.role === 'user' && getAvatar(m.role)}
          </div>
        ))}
