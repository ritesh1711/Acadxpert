...
/* ---------------- Style dictionary for inline styles ---------------- */
const css = {
  page: {
    minHeight: '100vh',
  ...
  list: { overflowY: 'auto', padding: '8px 0' } as React.CSSProperties,
  threadItem: (active: boolean): React.CSSProperties => ({
    padding: 16,
    cursor: 'pointer',
  ...
  name: { fontWeight: 600, fontSize: 16 } as React.CSSProperties,
  badge: {
    minWidth: 22,
    height: 22,
    borderRadius: 11,
  ...
  sub: { fontSize: 13, color: 'var(--muted)', marginTop: 2 } as React.CSSProperties,
  chatHead: {
    padding: 18,
    borderBottom: '1px solid var(--cardStroke)',
    display: 'flex',
  ...
  avatar: (seed: string): React.CSSProperties => ({
    width: 38,
    height: 38,
    borderRadius: '50%',
  ...
  bubbleRow: (isAdmin: boolean): React.CSSProperties => ({
    display: 'flex',
    justifyContent: isAdmin ? 'flex-end' : 'flex-start',
    animation: 'fadeInUp 180ms ease',
  }),
  bubble: (isAdmin: boolean): React.CSSProperties => ({
    maxWidth: 540,
    padding: '14px 18px',
    borderRadius: 18,
  ...
  meta: { fontSize: 12, color: '#64748b', marginTop: 6, textAlign: 'right' } as React.CSSProperties,
  typing: {
    display: 'inline-flex',
    gap: 4,
    alignItems: 'center',
  ...
  small: { fontSize: 13, color: '#64748b', fontWeight: 500 } as React.CSSProperties,
};
...
export default function AdminInbox() {
  ...
  const [active, setActive] = useState<{ studentId: string; student?: Thread['student'] } | null>(null);
