...
const Circulars = ({ course, semester }) => { 
  // course and semester should be passed from props or context
  ...
  const fetchCirculars = async () => {
    try {
      const res = await axios.get(`http://localhost:8000/admin/circulars`, {
        params: { course, semester } // send course and semester to backend
      });
  ...
  }, [course, semester]); // refetch when course/semester changes
  ...
  return (
    <div className="max-w-4xl mx-auto p-6 mt-10 bg-white shadow-lg rounded-xl border border-gray-200">
      ...
      {loading ? (
        ...
        <p className="text-center text-gray-500">No cixrculars available</p>
      ) : (
